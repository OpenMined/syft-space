  ...
  <div class="flex min-h-screen w-full flex-row overflow-hidden select-none">
    ...
    <div class="relative flex w-full flex-col justify-between p-8 md:w-2/5">
      ...
      <div class="relative z-10">
        ...
        <p class="mt-2 text-white/80">
          A Space where you can turn data and models into shareable workflows — exposing them through secure endpoints
          under your own rules for privacy, payments, and human oversight.
        </p>
      </div>
    ...
    <div class="bg-primary-foreground flex max-h-screen w-full flex-col md:w-3/5">
        ...
        <Transition enter-active-class="transition-all duration-200 ease-out" enter-from-class="opacity-0 translate-y-2"
          ...
          <div :key="state.updateWindowType">
            ...
            <div v-if="state.updateWindowType === UpdateType.error" class="flex items-center">
              <div class="mr-3 flex h-10 w-10 items-center justify-center rounded-full bg-red-100 dark:bg-red-900/30">
      ...
      <div :class="[
        ...
        state.updateWindowType === UpdateType.downloading ? 'flex flex-col items-center justify-center' : ''
      ]">
        <Transition enter-active-class="transition-opacity duration-200 ease-out" enter-from-class="opacity-0"
          ...
          <div :key="state.updateWindowType" class="w-full">
            <div v-if="state.updateWindowType === UpdateType.available && state.releaseNotes"
              class="prose prose-sm dark:prose-invert max-w-none select-auto">
              <VueMarkdown :source="state.releaseNotes" :options="{ linkify: true, breaks: true }"
                @click="handleMarkdownClick" />
            </div>

            <div v-if="state.updateWindowType === UpdateType.error && state.error"
            ...
            <div v-if="state.updateWindowType === UpdateType.failed && state.error"
              class="rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-900/20">
              <p class="break-words text-red-800 dark:text-red-300">
                {{ state.error }}
              </p>
            ...
            <div v-if="state.updateWindowType === UpdateType.downloading" class="mx-auto w-full max-w-md text-center">
              <div class="relative w-full pt-1">
                <div class="flex h-2 overflow-hidden rounded-full bg-blue-200 dark:bg-blue-900/30">
                  <div class="bg-blue-500 transition-all duration-500 ease-out"
            ...
            <div v-if="state.updateWindowType === UpdateType.none" class="overflow-hidden py-6 text-center">
              <Transition enter-active-class="transition-transform duration-300" enter-from-class="scale-0"
                enter-to-class="scale-100" appear>
                <div
              ...
              <h3 class="text-primary mt-6 text-xl font-medium">
                You're all set!
              </h3>
            ...
            <div v-if="state.updateWindowType === UpdateType.checking" class="overflow-hidden py-6 text-center">
              <div class="animate-spin">
                <div
      ...
      <div data-tauri-drag-region class="flex justify-end gap-3 border-t p-6">
        <Transition enter-active-class="transition-all duration-200 ease-out" enter-from-class="opacity-0 translate-y-2"
          enter-to-class="opacity-100 translate-y-0" leave-active-class="transition-all duration-200 ease-in"
          leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-2" mode="out-in">
          <div :key="state.updateWindowType" data-tauri-drag-region class="flex w-full justify-end gap-3">
            <template v-if="state.updateWindowType === UpdateType.available">
              <Button variant="outline" @click="onLater">
                Remind Me Later
              </Button>
              <Button @click="onUpdate">Update Now</Button>
            </template>

            <template v-if="[UpdateType.none, UpdateType.error, UpdateType.failed].includes(state.updateWindowType)">
              <Button @click="closeHandler">Close</Button>
            </template>

            <Button v-if="state.updateWindowType === UpdateType.downloading" disabled
              class="cursor-not-allowed opacity-50">
              Downloading...
            </Button>

            <Button v-if="state.updateWindowType === UpdateType.checking" disabled
              class="cursor-not-allowed opacity-50">
              Checking...
            </Button>
          </div>
...
watch(() => state.updateWindowType, (newType) => {
...
onMounted(async () => {
  if (window.__TAURI__) {
    // Get initial state
    const initialStateData = await window.__TAURI__.core.invoke<UpdateWindowState>(
      'get_window_state'
    )
