          ...
          <div class="space-y-6">
            ...
            <div class="space-y-2">
              ...
              <Input
                ...
                class="w-full"
              />
              <p class="text-sm text-gray-500">A short summary that will appear in endpoint listings</p>
            </div>
              ...
              <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div
                    ...
                    :key="model.id"
                    @click="model.isCustom ? openCustomSDKDocs() : selectedNewModelType = model.id"
                    :class="[
                      ...
                      model.isCustom 
                        ...
                        : (selectedNewModelType === model.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:bg-gray-50')
                    ]"
                  >
                    ...
                    <span v-else class="font-medium text-center" :class="selectedNewModelType === model.id ? 'text-blue-900' : 'text-gray-900'">
                      {{ model.name }}
                    </span>
                ...
                <RadioGroup v-model="formData.aiModel">
                  <div class="space-y-3">
                    <div
                      ...
                      :class="formData.aiModel === model.id ? 'border-green-500 bg-green-50' : 'border-gray-200'"
                      ...
                      <Label :for="model.id" class="flex items-center gap-3 cursor-pointer flex-1">
                        ...
                        <div class="flex-1">
                          <div class="flex items-center gap-2">
                            ...
                            <Badge variant="secondary" class="text-xs capitalize">{{ model.type }}</Badge>
                            <Badge 
                              variant="outline" 
        ...
        <div v-if="currentSubStep === 3" class="space-y-8">
          ...
          <div class="space-y-6">
            ...
            >
              ...
              <!-- Empty State -->
              <div
                v-if="policyRules[policy.id] && policyRules[policy.id].length === 0"
              ...
              <!-- Policy Rules -->
              <div v-if="policyRules[policy.id] && policyRules[policy.id].length > 0" class="space-y-4">
                <div
                ...
                >
                  ...
                  <div v-if="rule.isEditing" class="space-y-4">
                    ...
                    <div v-if="policy.id === 'authorization'" class="space-y-6">
                      ...
                      <div class="flex items-center justify-between">
                        <div>
                          <h3 class="text-lg font-medium text-gray-900">Authorization rule #{{ policyRules[policy.id].findIndex(r => r.id === rule.id) + 1 }}</h3>
                          <p class="text-sm text-gray-600 mt-1">
                            {{ authorizationForm.ruleType === 'allow' ? 'Only these users can access' : 'These users cannot access' }}
                          </p>
                        </div>
                      ...
                      <div class="space-y-2">
                        <Label for="auth-users">
                          {{ authorizationForm.ruleType === 'allow' ? 'Who can access?' : 'Who cannot access?' }}
                        </Label>
                        <p class="text-sm text-gray-600">Comma-separated list. Examples: jane@acme.com, *@acme.com</p>
                        <Input
                          id="auth-users"
                    ...
                    <div v-if="policy.id === 'ratelimiter'" class="space-y-6">
                      <div class="flex items-center justify-between">
                        <div>
                          <h3 class="text-lg font-medium text-gray-900">Rate limiter rule #{{ policyRules[policy.id].findIndex(r => r.id === rule.id) + 1 }}</h3>
                          <p class="text-sm text-gray-600 mt-1">Control request rates to prevent abuse and ensure fair resource usage</p>
                        </div>
                        <Button variant="ghost" size="sm" @click="cancelEdit(policy.id, rule.id)">
                          Remove
                        </Button>
                      ...
                      <div class="w-full space-y-6">
                        <div class="w-full grid grid-cols-1 md:grid-cols-5 gap-6">
                          <div class="col-span-3 space-y-2">
                            ...
                            <div class="w-full flex items-center gap-3">
                              ...
                              />
                              <span class="text-sm text-gray-600 whitespace-nowrap">requests per</span>
                              <Input
                                id="rate-window"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                          ...
                          <div v-if="rateLimiterForm.userType && rateLimiterForm.userType !== 'all'" class="col-span-2 space-y-2">
                            <Label for="rate-users">
                              {{ rateLimiterForm.userType === 'only' ? 'Which users' : 'Exclude which users' }}
                            </Label>
                            <Input
                              id="rate-users"
                              v-model="rateLimiterForm.users"
                              :placeholder="rateLimiterForm.userType === 'only' ? 'john@company.com, *@contractors.com' : 'admin@company.com, *@management.com'"
                              class="w-full"
                            />
                            <p class="text-xs text-gray-500">
                    ...
                    <div v-if="policy.id === 'pricing'" class="space-y-6">
                      ...
                      <div class="flex items-center justify-between">
                        <div>
                          <h3 class="text-lg font-medium text-gray-900">Pricing rule #{{ policyRules[policy.id].findIndex(r => r.id === rule.id) + 1 }}</h3>
                          <p class="text-sm text-gray-600 mt-1">Set pricing based on requests or tokens consumed</p>
                        </div>
                        <Button variant="ghost" size="sm" @click="cancelEdit(policy.id, rule.id)">
                          Remove
                        </Button>
                      ...
                      <div class="w-full space-y-6">
                          ...
                          <div class="col-span-3 space-y-2">
                            ...
                            <div class="w-full max-w-md flex items-center gap-3">
                              ...
                              <Input
                                ...
                                class="w-20 min-w-0"
                              />
                              <span class="text-sm text-gray-600 whitespace-nowrap">{{ pricingForm.pricingType === 'request' ? 'request(s)' : 'token(s)' }}</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                          ...
                          <div v-if="pricingForm.userType && pricingForm.userType !== 'all'" class="col-span-2 space-y-2">
                            <Label for="pricing-users">
                              {{ pricingForm.userType === 'only' ? 'Which users' : 'Exclude which users' }}
                            </Label>
                            <Input
                              id="pricing-users"
                              v-model="pricingForm.users"
                              :placeholder="pricingForm.userType === 'only' ? 'john@company.com, *@contractors.com' : 'admin@company.com, *@management.com'"
                              class="w-full"
                            />
                            <p class="text-xs text-gray-500">
                              Comma-separated list. Wildcard supported (e.g., *@company.com, *.edu, *@contractors.org)
                    ...
                    <div v-if="policy.id === 'manual-approval'" class="space-y-6">
                      ...
                      <div class="flex items-center justify-between">
                        <div>
                          <h3 class="text-lg font-medium text-gray-900">Manual approval rule #{{ policyRules[policy.id].findIndex(r => r.id === rule.id) + 1 }}</h3>
                          <p class="text-sm text-gray-600 mt-1">Require human approval for sensitive operations and decisions</p>
                        </div>
                        <Button variant="ghost" size="sm" @click="cancelEdit(policy.id, rule.id)">
                          Remove
                        </Button>
                      ...
                      <div v-else-if="manualApprovalForm.destination === 'slack'" class="space-y-2">
                        ...
                        />
                        <p class="text-xs text-gray-500">Your Slack webhook URL for notifications</p>
                      </div>
                      <div v-else-if="manualApprovalForm.destination === 'whatsapp'" class="space-y-2">
                      ...
                      <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                        ...
                        <div v-if="manualApprovalForm.userType && manualApprovalForm.userType !== 'all'" class="col-span-2 space-y-2">
                          <Label for="approval-users">
                            {{ manualApprovalForm.userType === 'only' ? 'Which users' : 'Exclude which users' }}
                          </Label>
                          <Input
                            id="approval-users"
                            v-model="manualApprovalForm.users"
                            :placeholder="manualApprovalForm.userType === 'only' ? 'john@company.com, *@contractors.com' : 'admin@company.com, *@management.com'"
                            class="w-full"
                          />
                          <p class="text-xs text-gray-500">
                            Comma-separated list. Wildcard supported (e.g., *@company.com, *.edu, *@contractors.org)
                    ...
                    <div v-if="policy.id === 'ai-filters'" class="space-y-6">
                      ...
                      <div class="flex items-center justify-between">
                        <div>
                          <h3 class="text-lg font-medium text-gray-900">AI filters rule #{{ policyRules[policy.id].findIndex(r => r.id === rule.id) + 1 }}</h3>
                          <p class="text-sm text-gray-600 mt-1">Filter or redact responses using an AI before sending them back</p>
                        </div>
                        <Button variant="ghost" size="sm" @click="cancelEdit(policy.id, rule.id)">
                          Remove
                        </Button>
                      ...
                      <div class="space-y-2">
                        ...
                        />
                        <p class="text-xs text-gray-500">This prompt will be applied to filter or transform the response before sending it back to the user</p>
                      </div>
                      ...
                      <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                        ...
                        <div v-if="aiFiltersForm.userType && aiFiltersForm.userType !== 'all'" class="col-span-2 space-y-2">
                          <Label for="filter-users">
                            {{ aiFiltersForm.userType === 'only' ? 'Which users' : 'Exclude which users' }}
                          </Label>
                          <Input
                            id="filter-users"
                            v-model="aiFiltersForm.users"
                            :placeholder="aiFiltersForm.userType === 'only' ? 'john@company.com, *@contractors.com' : 'admin@company.com, *@management.com'"
                            class="w-full"
                          />
                          <p class="text-xs text-gray-500">
                            Comma-separated list. Wildcard supported (e.g., *@company.com, *.edu, *@contractors.org)
                    ...
                    <div class="pt-4 border-t border-gray-200">
                      <div class="flex justify-end gap-2">
                        ...
                        <Button @click="savePolicy(policy.id, rule.id)">
                          Save Policy
                        </Button>
                      </div>
                    </div>
                  ...
                  <div v-else class="flex items-start justify-between">
                    <div class="flex-1 space-y-2">
                      <h4 class="text-sm font-medium text-gray-900">
                        {{ rule.config.note || `${policy.name} rule #${policyRules[policy.id].findIndex(r => r.id === rule.id) + 1}` }}
                      </h4>
                      <div class="text-sm text-gray-600">
                        <!-- Authorization Rule Summary -->
                        <p v-if="rule.config.ruleType">
                          {{ rule.config.ruleType === 'allow' ? 'Allow' : 'Deny' }} access to {{ rule.config.users || 'specified users' }}
                        </p>
                        
                        <!-- Rate Limiter Rule Summary -->
                        <div v-if="rule.config.limit">
                          <p>{{ rule.config.limit }} requests per {{ rule.config.windowValue }} {{ rule.config.windowUnit }}(s) {{ rule.config.scope }}</p>
                          <p v-if="rule.config.userType === 'only' && rule.config.users" class="text-xs text-gray-500">
                            Only for: {{ rule.config.users }}
                          </p>
                          <p v-if="rule.config.userType === 'except' && rule.config.users" class="text-xs text-gray-500">
                            Except for: {{ rule.config.users }}
                          </p>
                        </div>
                        
                        <!-- Pricing Rule Summary -->
                        <div v-if="rule.config.price !== undefined && rule.config.price !== null && rule.config.price !== ''">
                          <p>${{ rule.config.price }} per {{ rule.config.quantity }} {{ rule.config.pricingType }}(s)</p>
                          <p v-if="rule.config.userType === 'only' && rule.config.users" class="text-xs text-gray-500">
                            Only for: {{ rule.config.users }}
                          </p>
                          <p v-if="rule.config.userType === 'except' && rule.config.users" class="text-xs text-gray-500">
                        ...
                        <div v-if="rule.config.destination">
                          <p>Send alerts via {{ rule.config.destination === 'inbox' ? 'in-app notification' : rule.config.destination }}</p>
                          <p v-if="rule.config.destination === 'email' && rule.config.emailAddresses" class="text-xs text-gray-500">
                            To: {{ rule.config.emailAddresses }}
                          </p>
                          <p v-if="rule.config.destination === 'slack' && rule.config.slackWebhookUrl" class="text-xs text-gray-500">
                            <span class="font-medium">Webhook:</span>
                            <span class="font-mono bg-gray-100 px-1 py-0.5 rounded break-all">{{ rule.config.slackWebhookUrl }}</span>
                          </p>
                          <p v-if="rule.config.destination === 'whatsapp' && rule.config.whatsappNumber" class="text-xs text-gray-500">
                            Number: {{ rule.config.whatsappNumber }}
                          </p>
                          <p class="text-sm">Timeout: {{ rule.config.timeoutValue }} {{ rule.config.timeoutUnit }}(s)</p>
                          <p v-if="rule.config.userType === 'only' && rule.config.users" class="text-xs text-gray-500">
                            Only for: {{ rule.config.users }}
                          </p>
                          <p v-if="rule.config.userType === 'except' && rule.config.users" class="text-xs text-gray-500">
                        ...
                        <div v-if="rule.config.modelId">
                          <p>Filter using {{ mockModels.find(m => m.id === rule.config.modelId)?.name || rule.config.modelId }}</p>
                          <div v-if="rule.config.prompt" class="mt-2 text-xs text-gray-500">
                            <p class="font-medium mb-1">Prompt:</p>
                            <div class="font-mono bg-gray-50 px-2 py-1 rounded border max-h-20 overflow-y-auto whitespace-pre-wrap">{{ rule.config.prompt }}</div>
                          </div>
                          <p v-if="rule.config.userType === 'only' && rule.config.users" class="text-xs text-gray-500 mt-2">
                            Only for: {{ rule.config.users }}
                          </p>
                          <p v-if="rule.config.userType === 'except' && rule.config.users" class="text-xs text-gray-500 mt-2">
                            Except for: {{ rule.config.users }}
                          </p>
                        </div>
                      </div>
                    ...
                    <div class="flex gap-2 ml-4">
                      <Button variant="outline" size="sm" class="text-gray-600" @click="editPolicy(policy.id, rule.id)">
                        <Edit class="h-4 w-4 mr-2" />
                        Edit
                      </Button>
                      <Button variant="outline" size="sm" class="text-red-600 hover:text-red-700" @click="deletePolicy(policy.id, rule.id)">
              ...
              <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-sm text-gray-700 mb-4">
                    You can create your own custom policies for your specific needs using our SDK. Build policies tailored to your unique requirements and integrate them seamlessly.
                  </p>
                  <Button
                    variant="outline"
        ...
        <div v-if="currentSubStep === 4" class="space-y-8">
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
            <div class="space-y-6">
              <div>
                ...
                <p class="text-sm text-gray-600">Review your model endpoint configuration before deployment</p>
              </div>
              ...
              <div class="border-l-4 border-blue-500 pl-4">
                ...
                <div class="space-y-1 text-sm">
                  <p><span class="font-medium">Name:</span> {{ formData.endpointName || 'Not specified' }}</p>
              ...
              <div class="border-l-4 border-orange-500 pl-4">
                ...
                <div class="text-sm space-y-2">
                  <div v-if="Object.keys(getAppliedPoliciesGrouped()).length > 0" class="space-y-6">
                    <div v-for="(policyGroup, policyType) in getAppliedPoliciesGrouped()" :key="policyType" class="space-y-3">
                      <!-- Policy Type Header -->
                      <div class="flex items-center gap-3">
                        <div :class="[
                          'p-2 rounded-lg',
                        ...
                        <span class="text-xs text-gray-500">({{ policyGroup.rules.length }} rule{{ policyGroup.rules.length !== 1 ? 's' : '' }})</span>
                      </div>
                      
                      <!-- Policy Rules -->
                      <div class="space-y-3 ml-6">
                        <div v-for="rule in policyGroup.rules" :key="rule.id" class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                          ...
                          <div class="space-y-2 text-sm text-gray-700">
                            ...
                            <div v-if="rule.config.ruleType">
                              <p class="flex items-start">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Type:</span>
                                <span>{{ rule.config.ruleType === 'allow' ? 'Allow-list' : 'Deny-list' }}</span>
                              </p>
                              <p class="flex items-start">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Users:</span>
                            ...
                            <div v-if="rule.config.limit">
                              <p class="flex items-start">
                                ...
                                <span>{{ rule.config.limit }} requests per {{ rule.config.windowValue }} {{ rule.config.windowUnit }}(s)</span>
                              </p>
                              <p class="flex items-start">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Scope:</span>
                                <span>{{ rule.config.scope || 'Per user' }}</span>
                              </p>
                              <p v-if="rule.config.userType === 'only' && rule.config.users" class="flex items-start">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Users:</span>
                                <span>Only {{ rule.config.users }}</span>
                              </p>
                              <p v-if="rule.config.userType === 'except' && rule.config.users" class="flex items-start">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Users:</span>
                            ...
                            <div v-if="rule.config.price !== undefined && rule.config.price !== null && rule.config.price !== ''">
                              <p class="flex items-start">
                                ...
                                <span>${{ rule.config.price }} per {{ rule.config.quantity }} {{ rule.config.pricingType }}(s)</span>
                              </p>
                              <p v-if="rule.config.userType === 'only' && rule.config.users" class="flex items-start">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Users:</span>
                                <span>Only {{ rule.config.users }}</span>
                            ...
                            <div v-if="rule.config.destination">
                              <p class="flex items-start">
                                ...
                                <span class="capitalize">{{ rule.config.destination === 'inbox' ? 'In-app notification' : rule.config.destination }}</span>
                              </p>
                              <p v-if="rule.config.destination === 'email' && rule.config.emailAddresses" class="flex items-start">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Email:</span>
                                <span>{{ rule.config.emailAddresses }}</span>
                              </p>
                              <p v-if="rule.config.destination === 'slack' && rule.config.slackWebhookUrl" class="flex items-start">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Webhook:</span>
                                <span class="text-xs font-mono bg-gray-100 px-2 py-1 rounded break-all">{{ rule.config.slackWebhookUrl }}</span>
                              </p>
                              <p v-if="rule.config.destination === 'whatsapp' && rule.config.whatsappNumber" class="flex items-start">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Number:</span>
                                <span>{{ rule.config.whatsappNumber }}</span>
                              </p>
                              <p class="flex items-start">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Timeout:</span>
                                <span>{{ rule.config.timeoutValue }} {{ rule.config.timeoutUnit }}(s)</span>
                              </p>
                              <p v-if="rule.config.userType === 'only' && rule.config.users" class="flex items-start">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Users:</span>
                                <span>Only {{ rule.config.users }}</span>
                            ...
                            <div v-if="rule.config.modelId">
                              <p class="flex items-start">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Model:</span>
                                <span>{{ mockModels.find(m => m.id === rule.config.modelId)?.name || rule.config.modelId }}</span>
                              </p>
                              <div v-if="rule.config.prompt" class="mt-2">
                                <p class="font-medium text-gray-500 mb-2">Prompt:</p>
                                <div class="text-xs bg-white border rounded px-3 py-2 font-mono max-h-32 overflow-y-auto whitespace-pre-wrap">{{ rule.config.prompt }}</div>
                              </div>
                              <p v-if="rule.config.userType === 'only' && rule.config.users" class="flex items-start mt-2">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Users:</span>
                                <span>Only {{ rule.config.users }}</span>
                              </p>
                              <p v-if="rule.config.userType === 'except' && rule.config.users" class="flex items-start mt-2">
                                <span class="font-medium text-gray-500 w-20 flex-shrink-0">Users:</span>
                                <span>All except {{ rule.config.users }}</span>
                              </p>
                            </div>
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
            <div class="space-y-6">
              ...
              <div class="space-y-4">
                <!-- Public Endpoint -->
                <div class="flex items-center space-x-3 p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 hover:bg-gray-50"
                     :class="endpointVisibility === 'public' ? 'border-green-500 bg-green-50' : 'border-gray-200'"
                ...
                <!-- Private Endpoint -->
                <div class="flex items-center space-x-3 p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 hover:bg-gray-50"
                     :class="endpointVisibility === 'private' ? 'border-blue-500 bg-blue-50' : 'border-gray-200'"
                  ...
                  <div class="flex-1">
                    <label for="private-endpoint" class="cursor-pointer">
                      ...
                      <p class="text-sm text-gray-600">Only selected users can discover this endpoint.</p>
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div v-if="endpointVisibility === 'private'" class="space-y-4">
                <div>
                  ...
                  <p class="text-sm text-gray-600 mb-4">
                    Add email addresses of users who can discover this endpoint. You can leave this empty and add users later from the endpoint details page.
                  </p>
                </div>
...
import { ArrowLeft, Plus, X, ArrowRight, Save, CheckSquare, Square, Filter, ArrowUpDown, FolderOpen, Code, ExternalLink, Search, Shield, Gauge, DollarSign, UserCheck, Filter as FilterIcon, Edit, Trash2, Info, Globe, Lock } from 'lucide-vue-next'
...
const getAppliedPoliciesGrouped = () => {
        ...
        if (savedRules.length > 0) {
          groupedPolicies[policyType.name] = {
            ...
            rules: savedRules.map((rule, index) => ({
              id: rule.id,
              name: rule.config.note || `${policyType.name} rule #${index + 1}`,
              config: rule.config
            }))
          }
        }
...
const resetFormData = (policyId: string) => {
  switch (policyId) {
    case 'authorization':
      ...
      break
    case 'ratelimiter':
      rateLimiterForm.value = { limit: '', windowValue: '1', windowUnit: 'minute', scope: 'per user', type: 'sliding window', userType: 'all', users: '', note: '' }
      break
    case 'pricing':
      pricingForm.value = { pricingType: 'request', price: '', quantity: '1', userType: 'all', users: '', note: '' }
      break
    case 'manual-approval':
      manualApprovalForm.value = { destination: 'inbox', emailAddresses: '', slackWebhookUrl: '', whatsappNumber: '', timeoutValue: '24', timeoutUnit: 'hour', userType: 'all', users: '', note: '' }
      break
    case 'ai-filters':
...
watch(() => pricingForm.value.pricingType, (newType) => {
