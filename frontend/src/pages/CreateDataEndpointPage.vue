              ...
              <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  >
                    ...
                    />
                    <div v-if="db.isCustom" class="text-center transition-all duration-200 min-h-[1.25rem]">
                    ...
                    <span v-else class="text-sm font-medium text-center" :class="[
                      selectedVectorDB === db.id ? 'text-purple-900' : 'text-gray-900'
                    ]">
                      {{ db.name }}
                    </span>
                    <div v-if="db.isCustom" class="text-center transition-all duration-200 min-h-[1rem]">
                    ...
                    <div class="flex-1 space-y-2">
                      ...
                      <div class="text-sm text-gray-600">
                        ...
                        <div v-if="rule.config.limit">
                          ...
                          <p v-else-if="rule.config.userType === 'except' && rule.config.users" class="text-xs text-gray-500">
                            Excludes: {{ rule.config.users }}
                          </p>
                          <p v-else-if="rule.config.userType === 'all'" class="text-xs text-gray-500">
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
            <div class="space-y-6">
              <div>
                ...
                <p class="text-sm text-gray-600">Review your endpoint configuration before deployment</p>
              </div>
                ...
                <div class="text-sm space-y-1">
                  ...
                  <div v-if="(formData.responseType === 'summary' || formData.responseType === 'both')">
                    <div v-if="formData.aiModel && getSelectedModel()">
                      <p><span class="font-medium">AI Model:</span> {{ getSelectedModel().name }}</p>
                      <p><span class="font-medium">Provider:</span> {{ getSelectedModel().type }}</p>
                      <p><span class="font-medium">Status:</span> 
                        <Badge 
                          variant="outline" 
                          :class="getSelectedModel().status === 'running' 
                          ...
                          <div class="space-y-2 text-sm text-gray-700">
                            ...
                            <div v-if="rule.config.limit">
                              ...
                              <p class="flex items-start">
                                ...
                                <span v-else-if="rule.config.userType === 'only'">Only: {{ rule.config.users || 'No users specified' }}</span>
                                <span v-else-if="rule.config.userType === 'except'">Everyone except: {{ rule.config.users || 'No users specified' }}</span>
                              </p>
                            </div>
...
import { ArrowLeft, Database, Plus, X, ArrowRight, FileText, FolderOpen, Code, FileType, Sparkles, GitMerge, Save, ExternalLink, Search, Shield, Gauge, DollarSign, UserCheck, Filter, Edit, Trash2, Info, Globe, Lock } from 'lucide-vue-next'
...
const getFormPreviewTags = (policyId: string) => {
  let formData = {}
  ...
  for (const [key, value] of Object.entries(formData)) {
    if (value !== undefined && value !== null && value !== '' && key !== 'note' && String(value).trim() !== '') {
      // Special handling for AI filters modelId
      if (policyId === 'ai-filters' && key === 'modelId') {
        const model = mockModels.find(m => m.id === value)
        if (model) {
          tags.push(`model: ${model.name}`)
        } else if (value === 'create-new') {
          tags.push('model: New Model')
        }
      } else if (key === 'prompt') {
        // Truncate long prompts
        const truncated = value.length > 50 ? value.substring(0, 50) + '...' : value
        tags.push(`${key}: ${truncated}`)
      } else {
        tags.push(`${key}: ${value}`)
      }
    }
...
const getRuleConfigTags = (config: PolicyConfig) => {
  const tags = []
  for (const [key, value] of Object.entries(config)) {
    if (value !== undefined && value !== null && value !== '' && key !== 'note' && key !== 'id' && String(value).trim() !== '') {
      // Special handling for AI filters modelId
      if (key === 'modelId') {
        const model = mockModels.find(m => m.id === value)
        if (model) {
