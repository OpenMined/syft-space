    ...
    <ol class="flex items-center space-x-2">
      <li v-for="(item, index) in breadcrumbs" :key="index">
        ...
        <router-link
          v-if="item.route"
          :to="item.route"
          class="text-muted-foreground hover:text-foreground text-sm font-medium flex items-center"
        >
          <component v-if="item.icon" :is="item.icon" class="h-4 w-4 mr-1" />
          {{ item.label }}
        </router-link>

        <span v-else class="text-foreground text-sm font-medium flex items-center">
          <component v-if="item.icon" :is="item.icon" class="h-4 w-4 mr-1" />
          {{ item.label }}
        </span>
      </li>
