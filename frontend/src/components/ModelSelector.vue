    ...
    <RadioGroup :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)">
      <div class="space-y-3">
        ...
        <div v-for="model in mockModels" :key="model.id"
          ...
          <Label :for="`${idPrefix}-${model.id}`" class="flex items-center gap-3 cursor-pointer flex-1">
            <div :class="[
              'p-2 rounded',
            ...
            <div class="flex-1">
              <div class="flex items-center gap-2">
                ...
                <Badge variant="outline" :class="model.status === 'running'
                  ...
                  : 'bg-gray-50 text-gray-600 border-gray-200'" class="text-xs">
                  {{ model.status }}
                </Badge>
              </div>
        ...
        <div class="flex items-center space-x-3 p-4 border rounded-lg cursor-pointer hover:bg-gray-50 border-gray-200"
