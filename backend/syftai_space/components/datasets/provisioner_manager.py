...
class ProvisionerManager:
    """Lightweight manager for dataset provisioner lifecycle.

    Orchestrates startup and shutdown of provisioners across all datasets.
    All business logic is delegated to DatasetHandler.
    Uses asyncio.Event for clean shutdown coordination and task cancellation.
    """
    ...
    async def startup(self) -> None:
        """Start all provisioners in background (non-blocking).

        This method initiates provisioner startups but doesn't wait for them.
        The application can start serving requests while provisioners are starting.
        Tasks are automatically removed from tracking list when they complete.
        """
        ...
        logger.info("🚀 Initiating dataset provisioner startups...")

        # Get all datasets that have provisioners
        datasets = self.dataset_handler.get_all_datasets_with_provisioners()

        if not datasets:
            logger.info("No datasets with provisioners found")
            return

        # Start each provisioner in background
        for dataset in datasets:
            task = asyncio.create_task(self._start_provisioner_async(dataset))
    ...
    async def shutdown(self) -> None:
        """Stop all provisioners gracefully (blocking).
        ...
        logger.info("🛑 Shutting down dataset provisioners...")
        ...
        # Get fresh list in case state changed during startup cancellations
        datasets = self.dataset_handler.get_all_datasets_with_provisioners()

        if not datasets:
            logger.info("No datasets with provisioners to shut down")
            return

        # Stop all provisioners concurrently
        stop_tasks = []
        for dataset in datasets:
            task = asyncio.create_task(self._stop_provisioner_async(dataset))
        ...
        # Final cleanup: clear task list (tasks should already be removed by callbacks)
        self._startup_tasks.clear()

    async def _start_provisioner_async(self, dataset) -> None:
        ...
        try:
            ...
            await asyncio.to_thread(
                self.dataset_handler.start_provisioner_for_dataset, dataset
            )
        except asyncio.CancelledError:
            # Shutdown was called mid-startup - clean up partial resources
            logger.warning(
                f"⚠️  Startup cancelled for '{dataset.name}', attempting cleanup..."
            )
            try:
                # Best-effort cleanup: call stop to clean up any partial state
                await asyncio.to_thread(
                    self.dataset_handler.stop_provisioner_for_dataset, dataset
                )
                logger.info(f"✅ Cleanup successful for '{dataset.name}'")
            except Exception as e:
                logger.error(f"❌ Cleanup failed for '{dataset.name}': {e}")

            # Re-raise to signal cancellation completed
            raise
        except Exception as e:
            # Normal error during startup (already logged in handler)
            logger.error(f"❌ Provisioner startup failed for '{dataset.name}': {e}")
    ...
    async def _stop_provisioner_async(self, dataset) -> None:
        ...
        try:
            ...
            logger.error(f"❌ Provisioner shutdown failed for '{dataset.name}': {e}")
