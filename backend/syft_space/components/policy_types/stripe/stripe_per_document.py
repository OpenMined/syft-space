"""Stripe per-document policy type.
...
Behaviorally identical to ``XenditPerDocumentPolicy``; differs only in the
target charger and required wallet type.
"""
...
class StripePerDocumentPolicy(StripePaymentPolicy):
    """Stripe per-document pricing policy.
    ...
    CONFIG_CLS = StripePerDocumentConfig

    @classmethod
    def capabilities(cls) -> Capabilities:
        return Capabilities(
            requires_wallet=True,
            required_wallet_type="stripe",
    ...
    async def pre_hook(
        ...
        context.metadata["stripe_per_doc_price"] = price
    ...
    async def post_hook(
        ...
        price = context.metadata.get("stripe_per_doc_price")
        if price is None:
            return context
