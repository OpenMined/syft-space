"""Blogspot + ChromaDB dataset type binding.

Pairs the Blogger v3 source with an embedded ChromaDB vector store. The
...
# Credential fields stripped before a configuration is returned over the API.
# The blog URLs stay visible — they are public addresses, and seeing which
# blogs a dataset follows is the point.
_REDACTED_FIELDS = frozenset({"apiKey", "api_key"})


class BlogspotChromaDBConfiguration(BaseModel):
    """Flat user-facing configuration for the Blogspot binding.

    ``split_config`` translates this into per-axis configs at construction
    time.
    """
    ...
    api_key: str = Field(
        ...,
        alias="apiKey",
        description="Google API key with the Blogger API enabled",
...
class BlogspotChromaDBDatasetType(IngestableDatasetType):
    """Posts from public Blogspot blogs indexed in an embedded ChromaDB instance."""

    NAME: ClassVar[str] = "blogspot"
    SOURCE_PROVIDER_CLS: ClassVar[type[BlogspotProvider]] = BlogspotProvider
    ...
    def split_config(
        ...
        cfg = BlogspotChromaDBConfiguration.model_validate(configuration)
        source_cfg = {
            "blog_urls": cfg.blog_urls,
            "api_key": cfg.api_key,
    ...
    def configuration_schema(cls) -> dict[str, Any]:
        return BlogspotChromaDBConfiguration.model_json_schema(
    ...
    async def validate_configuration(cls, configuration: dict[str, Any]) -> None:
        ...
        try:
            BlogspotChromaDBConfiguration.model_validate(configuration)
    ...
    def redact_configuration(cls, configuration: dict[str, Any]) -> dict[str, Any]:
        """Drop the API key from the exposed configuration."""
        return {k: v for k, v in configuration.items() if k not in _REDACTED_FIELDS}


__all__ = [
    "BlogspotChromaDBConfiguration",
